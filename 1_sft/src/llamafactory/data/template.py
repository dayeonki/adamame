...
class ReasoningTemplate(Template):
    ...
    def encode_oneturn(
        ...
        has_open = self.thought_words[0].strip() in messages[-1]["content"]
        has_close = self.thought_words[1].strip() in messages[-1]["content"]

        if not has_open and has_close and not self.think_in_prompt:
            # current setup: prepend <think> into response so it is trained on
            messages[-1]["content"] = self.thought_words[0] + messages[-1]["content"]

        prompt_ids, response_ids = super().encode_oneturn(tokenizer, messages, system, tools)

        if not has_open and has_close and self.think_in_prompt:
            # alternative setup: append <think> to prompt side so it is masked
            prompt_ids += tokenizer.encode(self.thought_words[0], add_special_tokens=False)
        elif not has_open and not has_close:  # add empty cot
    ...
    def encode_multiturn(
        ...
        think_open_ids = tokenizer.encode(self.thought_words[0], add_special_tokens=False)
        needs_open = []  # track which assistant turns need <think> moved to prompt side
        for i in range(1, len(messages), 2):
            has_open = self.thought_words[0].strip() in messages[i]["content"]
            has_close = self.thought_words[1].strip() in messages[i]["content"]
            if not has_open and has_close:
                if self.think_in_prompt:
                    needs_open.append(i)  # will be appended to prompt after encoding
                else:
                    messages[i]["content"] = self.thought_words[0] + messages[i]["content"]

        encoded_messages = self._encode(tokenizer, messages, system, tools)
        for i in range(0, len(messages), 2):
            has_open = self.thought_words[0].strip() in messages[i + 1]["content"]
            has_close = self.thought_words[1].strip() in messages[i + 1]["content"]
            if (i + 1) in needs_open:
                # alternative setup: <think> goes on prompt side (masked)
                encoded_messages[i] += think_open_ids
            elif not has_open and not has_close:  # add empty cot
                if not self.enable_thinking:  # do not compute loss
...
def parse_template(tokenizer: "PreTrainedTokenizer") -> "Template":
    ...
    print("Chosen template class:", template_class.__name__)
...
def get_template_and_fix_tokenizer(tokenizer: "PreTrainedTokenizer", data_args: "DataArguments") -> "Template":
    ...
    if isinstance(template, ReasoningTemplate):
        ...
        template.think_in_prompt = data_args.think_in_prompt
