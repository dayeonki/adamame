...
def _build_lingua_detector():
    try:
        ...
        langs = [Language.ENGLISH, Language.FRENCH, Language.JAPANESE, Language.KOREAN, Language.PORTUGUESE, Language.THAI]
        ...
        return detector, lang_to_code
    except ImportError:
        logger.warning_rank0_once("lingua-language-detector not installed; language consistency logging disabled.")
        return None, {}
...
def _check_format(text: str) -> bool:
    """Return True if text has <think>...</think> followed by \\boxed{ somewhere after."""
    has_think = bool(_re.search(r"<think>.*?</think>", text, _re.DOTALL))
    after_think = _re.split(r"</think>", text, maxsplit=1)
    has_boxed = len(after_think) > 1 and "\\boxed{" in after_think[1]
    return has_think and has_boxed


def _detect_lang(detector, lang_to_code, text: str):
    if detector is None or not text:
        return None
    result = detector.detect_language_of(text)
    return lang_to_code.get(result) if result is not None else None


class WandbEvalTableCallback(TrainerCallback):
    r"""A callback for logging eval examples as a wandb table with language consistency and format checks."""

    def __init__(self, tokenizer, eval_dataset, num_samples: int = 8, max_new_tokens: int = 512) -> None:
        self.tokenizer = tokenizer
        self.eval_dataset = eval_dataset
        self.num_samples = num_samples
        self.max_new_tokens = max_new_tokens
        self._lingua_detector, self._lang_to_code = _build_lingua_detector()

    def _get_expected_lang(self, prompt_text: str):
        """Detect expected language from the user's question in the prompt."""
        # Extract text between <｜User｜> and <｜Assistant｜>
        m = _re.search(r"<｜User｜>(.*?)<｜Assistant｜>", prompt_text, _re.DOTALL)
        user_text = m.group(1).strip() if m else prompt_text
        return _detect_lang(self._lingua_detector, self._lang_to_code, user_text)
    ...
    def on_evaluate(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        table = wandb.Table(columns=["step", "prompt", "generated", "label", "expected_lang", "reasoning_lang", "lang_consistent", "format_ok"])

        lang_consistent_scores = []
        format_scores = []
        ...
        with torch.no_grad():
            for sample in samples:
                ...
                prompt_text = self.tokenizer.decode(
                    [t for t in sample["input_ids"] if t != self.tokenizer.pad_token_id],
                ...
                expected_lang = self._get_expected_lang(prompt_text)
                think_trace = _extract_think(generated_text)
                reasoning_lang = _detect_lang(self._lingua_detector, self._lang_to_code, think_trace)
                lang_consistent = int(expected_lang == reasoning_lang) if (expected_lang and reasoning_lang) else None
                if lang_consistent is not None:
                    lang_consistent_scores.append(lang_consistent)

                # format check
                format_ok = int(_check_format(generated_text))
                format_scores.append(format_ok)

                table.add_data(
                    state.global_step, prompt_text, generated_text, label_text,
                    expected_lang, reasoning_lang, lang_consistent, format_ok,
                )

        metrics = {"eval/examples": table}
        if lang_consistent_scores:
            metrics["eval/language_consistency"] = sum(lang_consistent_scores) / len(lang_consistent_scores)
        if format_scores:
            metrics["eval/format"] = sum(format_scores) / len(format_scores)
