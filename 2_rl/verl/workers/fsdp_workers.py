...
def resolve_attn_implementation(default: str = 'flash_attention_2') -> str:
    """
    Resolve attention backend with a safe fallback for clusters
    where flash-attn binary is not ABI-compatible.
    """
    requested = os.getenv('VERL_ATTN_IMPLEMENTATION', default)
    ...
    try:
        ...
        logger.warning(
            ...
            "Set VERL_ATTN_IMPLEMENTATION explicitly to override.",
            exc,
        )
        return 'sdpa'
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        # Apply LoRA if configured (actor only; ref uses full base model for KL).
        # Supports two config schemas:
        #   nested:  actor_rollout_ref.model.lora.rank=64  (verl docs / run_grpo.sh style)
        #   flat:    actor_rollout_ref.model.lora_rank=64  (SFT trainer style)
        lora_cfg_nested = self.config.model.get('lora', None)
        if lora_cfg_nested is not None and lora_cfg_nested.get('type', 'none') != 'none':
            lora_rank = lora_cfg_nested.get('rank', 0)
            lora_alpha = lora_cfg_nested.get('alpha', lora_rank)
            # nested target_modules may be null — fall back to flat key
            target_modules = lora_cfg_nested.get('target_modules', None) or \
                             self.config.model.get('target_modules', ['q_proj', 'v_proj'])
        else:
            lora_rank = self.config.model.get('lora_rank', 0)
            _lora_alpha = self.config.model.get('lora_alpha', lora_rank)
            lora_alpha = _lora_alpha if _lora_alpha > 0 else lora_rank
            target_modules = self.config.model.get('target_modules', ['q_proj', 'v_proj'])

        if lora_rank > 0 and role == 'actor':
            from peft import LoraConfig, TaskType, get_peft_model
            actor_module.enable_input_require_grads()
            if target_modules is None:
                target_modules = ['q_proj', 'v_proj']
            elif not isinstance(target_modules, (list, str)):
        ...
        # Enable gradient checkpointing after LoRA wrapping so hooks attach to the right model
        ...
        auto_wrap_policy = get_fsdp_wrap_policy(module=actor_module, config=fsdp_config.get('wrap_policy', None),
                                                is_lora=lora_rank > 0 and role == 'actor')
        ...
        actor_module_fsdp = FSDP(
            ...
            use_orig_params=lora_rank > 0 and role == 'actor',  # required for LoRA frozen-param handling
        ...
        if role == 'actor':
            ...
            params_to_optimize = [p for p in actor_module_fsdp.parameters() if p.requires_grad]
            actor_optimizer = optim.AdamW(params_to_optimize,
                                          lr=optim_config.lr,
