...
def load_fsdp_param_and_grad(module, device_id, load_grad=False):
    ...
    # When FSDP uses use_orig_params=True (required for LoRA) and world_size=1,
    # PyTorch internally uses NO_SHARD, so params are 2D natural-shape views into
    # the flat buffer.  The param.data = ... assignments above replace those views
    # with standalone tensors.  On the next forward, _writeback_orig_params detects
    # _same_storage is False and tries to copy the 2D param back to the 1D flat
    # buffer slot — but the expected_shape is always 1D, so it raises:
    #   "Cannot writeback when the parameter shape changes"
    # Re-calling _use_sharded_views() restores the correct flat-buffer views so
    # _same_storage is True again and the writeback is skipped entirely.
    # For multi-GPU FULL_SHARD (uses_sharded_strategy=True) the params are already
    # 1D shard views and the writeback shape check passes without this fix, so we
    # restrict to the no-shard case to leave multi-GPU runs untouched.
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
    for fsdp_mod in FSDP.fsdp_modules(module):
        handle = fsdp_mod._handle
        if handle is not None and handle._use_orig_params and not handle.uses_sharded_strategy:
